...
class GraphRAG():
    ...
    def create_entity_extract_chain(self):
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are extracting entities related to psychotherapy, counseling cases, emotional states, cognitive distortions, behavioral patterns, interpersonal interactions, self-perception, life events, cultural factors, and physical health entities from the text. Please ignore information unrelated to psychotherapy."
                ),
                (
                    "human",
    ...
    def create_vector_index(self,embedding_model) -> Neo4jVector:
        """
    ...
    def retriever(self, question: str) -> str:
        ...
        vector_index = self.create_vector_index(embedding_model=OpenAIEmbeddings())
    ...
    def create_search_query(self, chat_history: List, question: str) -> str:
        """
        让LLM 去决定应该用什么query 去检索的prompt，最好需要给一个例子
